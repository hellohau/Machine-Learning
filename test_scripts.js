...
const { Sequential } = require('@tensorflow/tfjs');
...
const xTrain3D = xTrain.reshape([4, 1, 2]);

// Define the model architecture
const model = tf.sequential();
model.add(tf.layers.simpleRNN({units: 4, inputShape: [1, 2]}));
...
// Train the model
model.fit(xTrain3D, yTrain, {epochs: 1000}).then(() => {
  // Test the model
  const xTest = tf.tensor2d([[0, 0], [0, 1], [1, 0], [1, 1]]);
  const xTest3D = xTest.reshape([4, 1, 2]);
  const yTest = model.predict(xTest3D);
  yTest.print();
});
